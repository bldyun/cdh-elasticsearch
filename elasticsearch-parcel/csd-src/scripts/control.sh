...
locateJava() {
    ...
    if [ -z ${JAVA_HOME} ];then
	export JAVA_HOME="$(echo /usr/java/jdk1.8* | cut -f1)"
    fi
    if [ -z ${JAVA_HOME} ];then
        export JAVA_HOME="$(echo /usr/java/jdk1.8* | cut -f1)"
    fi
	echo "Changing Java to: $JAVA_HOME"
    echo
}
...
init() {
	...
	sysctl -w vm.max_map_count=262144
        echo "elasticsearch soft nofile 65536" >/etc/security/limits.d/40-es-nproc.conf
        echo "elasticsearch hard nofile 131072" > /etc/security/limits.d/40-es-nproc.conf
        echo "elasticsearch soft nproc 4096" > /etc/security/limits.d/40-es-nproc.conf
        echo "elasticsearch hard nproc 4096" > /etc/security/limits.d/40-es-nproc.conf
...
start() {
    ...
    chown -RH elasticsearch:elasticsearch $ES_HOME
